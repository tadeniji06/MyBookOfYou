				...
				<Link href={`/read/${latestPost.slug.current}`}>
					<div className='group relative glass-panel rounded-3xl p-8 cursor-pointer overflow-hidden transition-all duration-500 hover:border-brand-red/50 hover:shadow-[0_0_40px_rgba(230,0,38,0.2)]'>
						...
						<div className='relative z-10 flex flex-col md:flex-row gap-6 items-start md:items-center'>
							<div className='bg-zinc-900/80 border border-zinc-800/50 p-4 rounded-2xl group-hover:scale-110 transition-transform duration-500 flex-shrink-0 backdrop-blur-sm'>
								{latestPost.mainImage ? (
									<div className='w-14 h-14 rounded-xl overflow-hidden bg-zinc-800 shadow-inner'>
										<img
							...
							<div className='flex-1 space-y-2'>
								<div className='flex items-center gap-3'>
									...
									<span className='h-1.5 w-1.5 rounded-full bg-brand-red animate-pulse shadow-[0_0_10px_rgba(230,0,38,0.8)]' />
								</div>
								<h3 className='text-3xl font-bold text-white group-hover:text-gradient-red transition-all duration-300 leading-tight'>
									{latestPost.title}
								</h3>
								<p className='text-zinc-500 text-sm line-clamp-2 leading-relaxed group-hover:text-zinc-300 transition-colors'>
				...
				<div className='opacity-80 hover:opacity-100 transition-opacity duration-500'>
					...
					<div className='grid grid-cols-1 md:grid-cols-2 gap-5'>
						{previousPosts.map((post) => (
							...
							>
								<div className='group glass-panel rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1 hover:border-brand-red/30 hover:shadow-[0_10px_30px_rgba(230,0,38,0.1)]'>
									<div className='flex justify-between items-start mb-4'>
										<span className='text-zinc-500 text-[10px] font-mono border border-zinc-800/50 bg-black/30 px-3 py-1 rounded-full tracking-wider'>
											{new Date(
												post.publishedAt,
											).toLocaleDateString()}
										</span>
									</div>
									<h5 className='text-lg font-semibold text-zinc-200 group-hover:text-brand-red transition-colors line-clamp-2 leading-snug'>
										{post.title}
									</h5>
								</div>
							</Link>
